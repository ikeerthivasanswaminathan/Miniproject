...
from firebase_service import initialize_firebase, update_bus_location, get_all_bus_locations
import time
...
def show_buses_page():
        ...
        if submit or add_another:
            if not all([driver_name, driver_phone, bus_name, bus_number, bus_route_number]):
                ...
                try:
                    ...
                    try:
                        initialize_firebase()
                        update_bus_location(bus_number, current_latitude, current_longitude)
                    except Exception as e:
    ...
    st.markdown("### 🚍 Registered Buses & Live Location Updates")
            ...
            for bus in buses:
                ...
                st.markdown(f"""
                    <div class='record-card'>
                        ...
                        <p><strong>Location:</strong> {live_lat:.4f}, {live_lng:.4f}</p>
                    </div>
                """, unsafe_allow_html=True)
                
                with st.expander(f"📍 Update {bus.bus_name} Location"):
                    col1, col2, col3 = st.columns([2, 2, 1])
                    
                    with col1:
                        new_lat = st.number_input(
                            "Latitude",
                            value=float(live_lat),
                            format="%.6f",
                            key=f"lat_{bus.id}"
                        )
                    ...
                    with col3:
                        st.write("")
                        st.write("")
                        if st.button("🔄 Update", key=f"update_{bus.id}"):
                            try:
                                initialize_firebase()
                                if update_bus_location(bus.bus_number, new_lat, new_lng):
