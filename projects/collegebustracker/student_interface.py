...
from firebase_service import initialize_firebase, get_bus_location, get_all_bus_locations
import time
...
def show_buses_page():
    ...
    try:
        ...
        if st.session_state.selected_bus is None:
            ...
            with col2:
                if st.button("🔄 Refresh", key="refresh_location"):
                    st.rerun()
            ...
            try:
                initialize_firebase()
                firebase_loc = get_bus_location(bus.bus_number)
                
                if firebase_loc and firebase_loc['latitude'] and firebase_loc['longitude']:
                    live_lat = firebase_loc['latitude']
                    live_lng = firebase_loc['longitude']
                    
                    if live_lat != bus.current_latitude or live_lng != bus.current_longitude:
                        bus.current_latitude = live_lat
                        bus.current_longitude = live_lng
                        db.commit()
                else:
                    live_lat = bus.current_latitude
                    live_lng = bus.current_longitude
            except:
                live_lat = bus.current_latitude
                live_lng = bus.current_longitude
            ...
            folium.Marker(
                [live_lat, live_lng],
                popup=f"<b>{bus.bus_name}</b><br>Route {bus.bus_route_number}<br>Last Updated: Just Now",
                tooltip=f"{bus.bus_name} (Live)",
                icon=folium.Icon(color='red', icon='bus', prefix='fa')
            ).add_to(m)
            
            st_folium(m, width=700, height=500)
            
            st.info("🔄 The map shows live bus location. Click 'Refresh' to update the position.")
            ...
            with col2:
                st.markdown(f"""
                    ...
                    <div class='info-row'>
                        ...
                        <span class='info-value'>{live_lat:.4f}, {live_lng:.4f}</span>
                    </div>
                    <div class='info-row'>
...
def check_bus_proximity():
    ...
    try:
        ...
        try:
            initialize_firebase()
            firebase_locations = get_all_bus_locations()
        except:
            firebase_locations = {}
        ...
        for bus in buses:
            firebase_loc = firebase_locations.get(bus.bus_number, {})
            live_lat = firebase_loc.get('latitude', bus.current_latitude)
            live_lng = firebase_loc.get('longitude', bus.current_longitude)
            ...
            bus_location = (live_lat, live_lng)
